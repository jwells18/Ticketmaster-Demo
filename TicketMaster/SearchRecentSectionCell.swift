...
protocol SearchRecentSectionCellDelegate {
...
class SearchRecentSectionCell: UITableViewCell, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, SearchRecentCellDelegate{
    
    var searchRecentSectionCellDelegate: SearchRecentSectionCellDelegate!
    ...
    func setupCollectionView(){
        collectionView.register(SearchRecentCell.self, forCellWithReuseIdentifier: cellIdentifier)
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout,sizeForItemAt indexPath: IndexPath) -> CGSize{
        return CGSize(width: collectionView.frame.width*0.5, height: collectionView.frame.height-20)
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellIdentifier, for: indexPath) as! SearchRecentCell
        cell.searchRecentCellDelegate = self
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        searchRecentSectionCellDelegate.didPressEventCell(cell: self, indexPath: indexPath)
    }
    
    //Button Delegates
    func viewMoreButtonPressed(sender: UIButton){
        searchRecentSectionCellDelegate.didPressViewMore(sender: sender)
    }
    
    func relayDidPressFavorite(sender: UIButton) {
        searchRecentSectionCellDelegate.didPressFavorite(sender: sender)
    }
}
